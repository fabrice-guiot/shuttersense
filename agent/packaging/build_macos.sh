...
#
# Output structure: dist/{version}/shuttersense-agent-darwin-{arch}
# where arch is arm64 (Apple Silicon) or amd64 (Intel)
...
if [ -f "$BINARY_PATH" ]; then
    ...
    FILE_SIZE=$(stat -f%z "$BINARY_PATH" 2>/dev/null || stat --printf="%s" "$BINARY_PATH" 2>/dev/null || echo "0")
