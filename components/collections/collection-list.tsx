...
export function CollectionList({
    ...
    <div className="flex flex-col items-center justify-center py-12 text-center">
      {search ? (
        <>
          ...
          <p className="text-muted-foreground">No collections matching &quot;{search}&quot;</p>
  ...
  const columns: ColumnDef<Collection>[] = [
    ...
    },
    {
      ...
      cell: (collection) => (
        <div className="flex items-center gap-2">
          <Badge variant="secondary">{COLLECTION_TYPE_LABELS[collection.type]}</Badge>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex items-center gap-1.5">
                  ...
                  />
                  <span className="text-sm truncate max-w-[100px]">{collection.bound_agent.name}</span>
                </div>
              </TooltipTrigger>
              <TooltipContent>
                <div className="text-xs">
                  <div className="font-medium">{collection.bound_agent.name}</div>
                  <div className="text-muted-foreground capitalize">Status: {collection.bound_agent.status}</div>
                </div>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
      ...
      cell: (collection) =>
        collection.pipeline_name ? (
          <span className="text-sm" title={`v${collection.pipeline_version}`}>
            {collection.pipeline_name}
            <span className="text-muted-foreground text-xs ml-1">v{collection.pipeline_version}</span>
          </span>
        ) : (
            ...
            <Tooltip>
              ...
              <TooltipContent>
                <div className="text-xs space-y-1">
                  <div>{collection.file_info.count.toLocaleString()} files cached</div>
    ...
    },
    {
      header: "Modified",
      cell: (collection) => (
        <span className="text-muted-foreground text-sm">{formatRelativeTime(collection.updated_at)}</span>
      ),
      ...
      cell: (collection) => (
        <div className="flex justify-end gap-1">
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" onClick={() => onInfo(collection)} aria-label="Test Accessibility">
  ...
  const renderTable = (tabCollections: Collection[]) => (
    <ResponsiveTable data={tabCollections} columns={columns} keyField="guid" emptyState={emptyState} />
  )
              ...
              {deleteDialog.forceDelete ? (
                <>
                  ...
                  <ul className="mt-2 list-disc list-inside text-sm">
                    {(deleteDialog.resultCount ?? 0) > 0 && <li>{deleteDialog.resultCount} analysis result(s)</li>}
                    {(deleteDialog.jobCount ?? 0) > 0 && <li>{deleteDialog.jobCount} active job(s)</li>}
                  </ul>
                  <p className="mt-2">
                    Deleting this collection will permanently remove all associated data. This action cannot be undone.
                  </p>
                </>
              ) : (
                <>
                  Are you sure you want to delete &quot;{deleteDialog.collection?.name}&quot;? This action cannot be
                  undone.
                </>
              )}
            </DialogDescription>
