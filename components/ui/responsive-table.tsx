...
export function ResponsiveTable<T>({
  ...
  const detailCols = columns.filter((c) => !c.cardRole || c.cardRole === "detail")
          ...
          >
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                {badgeCols.length > 0 && (
                  <div className="flex flex-wrap items-center gap-1 shrink-0">
                    {badgeCols.map((col) => (
                      <React.Fragment key={col.header}>{col.cell(item)}</React.Fragment>
                    ))}
                  </div>
                )}
              </div>
            ...
            {subtitleCols.length > 0 && (
              <div className="mt-1">
                {subtitleCols.map((col) => (
                  <div key={col.header} className="text-sm text-muted-foreground truncate">
                    {col.cell(item)}
                  </div>
                ))}
              </div>
            ...
            {detailCols.length > 0 && (
              <div
                className={cn(
                  "space-y-1",
                  (titleCols.length > 0 || subtitleCols.length > 0 || badgeCols.length > 0) &&
                    "mt-3 border-t border-border pt-3"
                )}
              >
                {detailCols.map((col) => (
                  <div key={col.header} className="flex items-center justify-between gap-2 text-sm">
