...
The system must verify that agents are authentic (from the official codebase) and that their results have not been tampered with.

#### Problem Statement

1. **Agent Authenticity**: How do we know the agent binary is legitimate and not a malicious actor?
2. **Job Claim Authorization**: How do we prevent authenticated-but-unauthorized clients from claiming jobs?
3. **Result Integrity**: How do we verify that submitted results were generated by a legitimate agent execution?
...
│                                                                              │
│  Source Code ──> Build ──> Sign Binary ──> Publish                          │
│                              │                                               │
│                              ▼                                               │
...
│                    - public_key_id: release-key-2024                         │
└─────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                           AGENT REGISTRATION                                 │
│                                                                              │
│  Agent sends:                         Server verifies:                       │
│  - registration_token                 - Token is valid and unused            │
│  - agent_version                      - Version exists in release manifest   │
│  - binary_checksum (self-hash)        - Checksum matches manifest            │
│                                       - (Optional) Binary signature valid    │
│                                                                              │
│  Server responds:                                                            │
│  - agent_guid                                                                │
│  - api_key (shown once)                                                      │
│  - signing_secret (for result HMAC) ◄── Used to sign job results            │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### Agent Identity Verification

During registration, the agent proves its authenticity:

```python
class AgentRegistrationRequest(BaseModel):
    """Agent registration with attestation."""
    registration_token: str
    name: str
    hostname: str

    # Attestation fields
    agent_version: str          # e.g., "v1.2.3"
    binary_checksum: str        # SHA-256 of agent binary
    platform: str               # e.g., "darwin-arm64", "linux-amd64"

def verify_agent_attestation(request: AgentRegistrationRequest) -> bool:
    """Verify agent binary is from official release."""
    # 1. Look up release manifest for this version
    manifest = get_release_manifest(request.agent_version)
    if not manifest:
        raise InvalidAgentVersion(f"Unknown version: {request.agent_version}")

    # 2. Get expected checksum for this platform
    expected_checksum = manifest.checksums.get(request.platform)
    if not expected_checksum:
        raise UnsupportedPlatform(f"No release for: {request.platform}")

    # 3. Verify checksum matches
    if request.binary_checksum != expected_checksum:
        raise AttestationFailed("Binary checksum mismatch - agent may be modified")

    return True
```

**Agent Self-Hash Computation:**
```python
# Agent computes its own checksum at startup
def compute_self_checksum() -> str:
    """Compute SHA-256 of the agent binary."""
    import hashlib
    agent_binary_path = Path(sys.executable)  # or bundled binary path
    with open(agent_binary_path, "rb") as f:
        return hashlib.sha256(f.read()).hexdigest()
```

#### Job Claim Authorization

Only verified agents can claim jobs:

```python
def claim_job(agent: Agent, capabilities: list[str]) -> Job | None:
    """Claim job - only for attested agents."""

    # 1. Verify agent is still trusted (not revoked)
    if agent.status == AgentStatus.REVOKED:
        raise AgentRevoked("Agent has been revoked")

    # 2. Verify agent attestation is still valid
    if not agent.attestation_verified:
        raise AttestationRequired("Agent must re-attest")

    # 3. Proceed with normal job claim logic
    # ... (existing claim logic)
```

#### Result Signing (HMAC)

When an agent claims a job, the server provides a job-specific signing token. Results must include a valid HMAC signature:

```python
# Server: When assigning job
def assign_job(job: Job, agent: Agent) -> JobAssignment:
    """Assign job with signing context."""
    # Generate job-specific signing secret
    job_signing_secret = generate_secret()
    job.signing_secret_hash = hash_secret(job_signing_secret)

    return JobAssignment(
        job=job,
        signing_secret=job_signing_secret,  # Sent once, agent stores temporarily
    )

# Agent: When submitting results
def sign_result(result_json: str, report_html: str, signing_secret: str) -> str:
    """Generate HMAC signature for results."""
    import hmac
    import hashlib

    # Canonical payload for signing
    payload = f"{result_json}|{hashlib.sha256(report_html.encode()).hexdigest()}"
    signature = hmac.new(
...
# Server: When receiving results
def verify_result_signature(job: Job, completion: JobCompletionRequest) -> bool:
    """Verify result was signed by the assigned agent."""
    expected_sig = compute_expected_signature(
        job.signing_secret_hash,
        completion.results_json,
        completion.report_html
    )
    return hmac.compare_digest(completion.signature, expected_sig)
```

#### Trust Revocation

Agents can be revoked if compromised:
...
class AgentStatus(str, Enum):
    ...
    BUSY = "busy"
    OFFLINE = "offline"
    REVOKED = "revoked"  # Cannot claim jobs, results rejected

# Admin action: Revoke compromised agent
def revoke_agent(agent_guid: str, reason: str) -> None:
    """Revoke agent - immediate effect."""
    agent = get_agent(agent_guid)
    agent.status = AgentStatus.REVOKED
    agent.revocation_reason = reason
    agent.revoked_at = datetime.utcnow()

    # Cancel any in-progress jobs
    for job in agent.assigned_jobs:
        if job.status in (JobStatus.ASSIGNED, JobStatus.RUNNING):
            job.status = JobStatus.PENDING  # Return to queue
            job.agent_id = None
```

### Result Integrity Guarantees

| Layer | What It Verifies | When |
|-------|------------------|------|
| HTTPS/TLS | Transport not tampered | Every request |
| API Key | Agent is authenticated | Every request |
| Attestation | Agent binary is official | Registration |
| HMAC Signature | Result from assigned agent | Job completion |
| Schema Validation | Result format is valid | Job completion |
| Anomaly Detection | Result content is plausible | Job completion (v2) |
...
Large analysis results (HTML reports can exceed 10MB) require a chunked upload protocol to avoid API payload limits and ensure reliable delivery.

#### Size Constraints

| Content Type | Typical Size | Max Size | Strategy |
|--------------|--------------|----------|----------|
| Results JSON | 10KB - 500KB | 5MB | Single POST if < 1MB, chunked otherwise |
| HTML Report | 100KB - 20MB | 50MB | Always chunked upload |
| Combined | 110KB - 20.5MB | 55MB | Separate uploads |
...
Agent                                  Server
  │                                      │
  │  1. POST /jobs/{guid}/complete       │
  │     { status: "COMPLETED",           │
  │       results_json: "...",           │ ◄── JSON inline (usually small)
  │       report_size: 15728640,         │ ◄── Declare HTML size (15MB)
  │       report_checksum: "sha256:...", │ ◄── For integrity verification
  │       signature: "hmac:..." }        │
  │  ─────────────────────────────────►  │
  │                                      │
  │  2. 202 Accepted                     │
  │     { upload_id: "upl_abc123",       │
  │       chunk_size: 5242880,           │ ◄── Server-specified chunk size (5MB)
  │       upload_url: "/uploads/upl_..." }│
  │  ◄─────────────────────────────────  │
  │                                      │
  │  3. PUT /uploads/{upload_id}/0       │
  │     [bytes 0-5242879]                │
  │  ─────────────────────────────────►  │
  │     200 OK { received: 5242880 }     │
  │  ◄─────────────────────────────────  │
  │                                      │
  │  4. PUT /uploads/{upload_id}/1       │
  │     [bytes 5242880-10485759]         │
  │  ─────────────────────────────────►  │
  │     200 OK { received: 10485760 }    │
  │  ◄─────────────────────────────────  │
  │                                      │
  │  5. PUT /uploads/{upload_id}/2       │
  │     [bytes 10485760-15728639]        │
  │  ─────────────────────────────────►  │
  │     200 OK { received: 15728640 }    │
  │  ◄─────────────────────────────────  │
  │                                      │
  │  6. POST /uploads/{upload_id}/finalize│
  │  ─────────────────────────────────►  │
  │                                      │  Server:
  │                                      │  - Reassembles chunks
  │                                      │  - Verifies checksum
  │                                      │  - Stores in AnalysisResult
  │  7. 200 OK                           │
  │     { job_status: "COMPLETED",       │
  │       result_guid: "res_xyz789" }    │
  │  ◄─────────────────────────────────  │
```
...
```python
# Server: Chunked upload endpoints
class ChunkedUploadService:
    """Manages chunked result uploads."""

    def initiate_upload(
        self,
        job: Job,
        content_type: str,  # "report_html" or "results_json"
        total_size: int,
        checksum: str
    ) -> ChunkedUpload:
        ...
        upload = ChunkedUpload(
            ...
            chunk_size=self.compute_chunk_size(total_size),
            chunks_received=[],
    ...
    def receive_chunk(
        ...
        """Store chunk and track progress."""
        upload = self.get_upload(upload_id)

        # Validate chunk
        expected_offset = chunk_index * upload.chunk_size
        if chunk_index in upload.chunks_received:
            raise ChunkAlreadyReceived(chunk_index)

        # Store chunk (filesystem or object storage)
        chunk_path = self.chunk_storage_path(upload_id, chunk_index)
        ...
        return ChunkReceipt(
            ...
            complete=(upload.bytes_received >= upload.total_size)
        )

    def finalize_upload(self, upload_id: str) -> bytes:
        """Reassemble chunks and verify integrity."""
        upload = self.get_upload(upload_id)

        # Reassemble
        assembled = bytearray()
        for i in sorted(upload.chunks_received):
            chunk_path = self.chunk_storage_path(upload_id, i)
            assembled.extend(chunk_path.read_bytes())

        # Verify checksum
        actual_checksum = f"sha256:{hashlib.sha256(assembled).hexdigest()}"
        if actual_checksum != upload.expected_checksum:
            raise ChecksumMismatch(upload.expected_checksum, actual_checksum)
...
All submitted results undergo validation before storage:

```python
class ResultValidator:
    """Validates agent-submitted results."""

    def validate_results_json(self, results_json: str, tool: str) -> ValidationResult:
        """Validate JSON results against tool-specific schema."""
        try:
            data = json.loads(results_json)
        except json.JSONDecodeError as e:
            return ValidationResult(valid=False, error=f"Invalid JSON: {e}")

        # Load tool-specific schema
        schema = self.get_schema_for_tool(tool)
        try:
            jsonschema.validate(data, schema)
        except jsonschema.ValidationError as e:
            return ValidationResult(valid=False, error=f"Schema violation: {e.message}")

        # Tool-specific semantic validation
        if tool == "photostats":
            return self.validate_photostats_results(data)
        elif tool == "photo_pairing":
            return self.validate_pairing_results(data)
        elif tool == "pipeline_validation":
            return self.validate_pipeline_results(data)

        return ValidationResult(valid=True)

    def validate_report_html(self, report_html: str, tool: str) -> ValidationResult:
        """Validate HTML report structure and safety."""
        # 1. Size check
        if len(report_html) > MAX_REPORT_SIZE:
            return ValidationResult(valid=False, error="Report exceeds size limit")

        # 2. Parse HTML
        try:
            soup = BeautifulSoup(report_html, 'html.parser')
        except Exception as e:
            return ValidationResult(valid=False, error=f"Invalid HTML: {e}")

        # 3. Security: No external scripts or dangerous elements
        dangerous_tags = soup.find_all(['script', 'iframe', 'object', 'embed'])
        for tag in dangerous_tags:
            if tag.name == 'script' and tag.get('src'):
                # External scripts not allowed
                return ValidationResult(valid=False, error="External scripts not allowed")

        # 4. Verify expected structure (tool generated this report)
        if not soup.find('meta', {'name': 'generator', 'content': re.compile(r'^photo-admin')}):
            return ValidationResult(valid=False, error="Missing generator meta tag")

        # 5. Verify report contains expected sections
        required_sections = self.get_required_sections(tool)
        for section_id in required_sections:
            if not soup.find(id=section_id):
                return ValidationResult(
                    valid=False,
                    error=f"Missing required section: {section_id}"
                )
...
```python
# Agent: Robust upload with retry
async def upload_report_with_retry(
    ...
    """Upload report with automatic retry and resumption."""
    chunks = [
        report_html[i:i + chunk_size]
        for i in range(0, len(report_html), chunk_size)
    ]
    ...
    for chunk_index, chunk_data in enumerate(chunks):
        ...
        if retry_count >= MAX_RETRIES:
            raise UploadFailed(f"Failed to upload chunk {chunk_index}")
```

---

### API-Based Tool Configuration

Tools currently support two configuration modes:
1. **Local YAML file** - Standalone CLI usage
2. **Database direct** - Server-side execution with DB connection

Agent execution requires a third mode: **API-based configuration** where tools fetch their configuration from the server via API.
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  Mode 1: LOCAL YAML                   Mode 2: DATABASE                       │
│  ┌─────────────────────┐              ┌─────────────────────┐               │
│  │ config/config.yaml  │              │ PostgreSQL DB       │               │
│  │ - photo_extensions  │              │ - settings table    │               │
│  │ - camera_mappings   │              │ - pipelines table   │               │
│  │ - pipelines/*.yaml  │              │ - collections table │               │
│  └──────────┬──────────┘              └──────────┬──────────┘               │
│             │                                    │                           │
│             ▼                                    ▼                           │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                     ConfigLoader (Abstract Interface)                │   │
│  │  - get_photo_extensions() -> list[str]                               │   │
│  │  - get_camera_mappings() -> dict                                     │   │
│  │  - get_pipeline(name: str) -> PipelineDefinition                     │   │
│  │  - get_collection(guid: str) -> CollectionConfig                     │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│             ▲                                    ▲                           │
│             │                                    │                           │
│  ┌──────────┴──────────┐              ┌──────────┴──────────┐               │
│  │ YamlConfigLoader    │              │ DatabaseConfigLoader │               │
│  │ (reads local files) │              │ (SQLAlchemy ORM)    │               │
│  └─────────────────────┘              └─────────────────────┘               │
│                                                                              │
│  Mode 3: API (NEW)                                                          │
│  ┌─────────────────────┐                                                    │
...
│             ▼                                                                │
│  ┌─────────────────────┐                                                    │
│  │ ApiConfigLoader     │  ◄── NEW: Agent uses this                          │
│  │ (HTTP client)       │                                                    │
│  └─────────────────────┘                                                    │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
...
class ConfigLoader(Protocol):
    ...
    Implementations:
    - YamlConfigLoader: Reads from local YAML files
    - DatabaseConfigLoader: Reads from database via ORM
    - ApiConfigLoader: Reads from server via HTTP API
    """
    ...
    def get_metadata_extensions(self) -> list[str]:
        """Get list of metadata/sidecar extensions."""
        ...
    ...
    def get_collection(self, collection_guid: str) -> CollectionConfig:
        """Get collection configuration by GUID."""
        ...

    def get_connector_config(self, connector_guid: str) -> ConnectorConfig:
        """Get connector configuration (credentials resolved separately)."""
        ...
```

#### API Config Loader Implementation

```python
class ApiConfigLoader:
    """Load configuration from server API.

    Used by agents to fetch configuration without direct database access.
    """
    ...
    def get_photo_extensions(self) -> list[str]:
        """Fetch photo extensions from server."""
        return self._cached_get("/api/config/photo-extensions")

    def get_camera_mappings(self) -> dict[str, CameraInfo]:
        """Fetch camera mappings from server."""
        data = self._cached_get("/api/config/camera-mappings")
    ...
    def get_collection(self, collection_guid: str) -> CollectionConfig:
        """Fetch collection configuration from server."""
        data = self.api_client.get(f"/api/collections/{collection_guid}")
        return CollectionConfig.from_api_response(data)

    def _cached_get(self, endpoint: str) -> Any:
        """Get with local caching to reduce API calls."""
        ...
        self._cache[endpoint] = (data, datetime.utcnow())
        return data
```
...
```python
# Backend: New configuration endpoints for agent consumption

@router.get("/api/config/photo-extensions")
async def get_photo_extensions(
    current_user: User = Depends(get_current_user)
) -> list[str]:
    """Get photo extensions for user's team."""
    settings = await get_team_settings(current_user.team_id)
    return settings.photo_extensions

@router.get("/api/config/camera-mappings")
async def get_camera_mappings(
    current_user: User = Depends(get_current_user)
) -> dict[str, CameraMappingResponse]:
    """Get camera mappings for user's team."""
    mappings = await get_team_camera_mappings(current_user.team_id)
    return {m.camera_id: CameraMappingResponse.from_orm(m) for m in mappings}

@router.get("/api/config/processing-methods")
...
async def get_job_config(
    ...
    """Get all configuration needed to execute a job.

    Returns collection config, pipeline definition, and tool settings
    in a single request to minimize round-trips.
    """
    job = await get_job_for_agent(job_guid, current_agent)

    return JobConfigResponse(
        collection=CollectionConfigResponse.from_orm(job.collection),
        pipeline=PipelineResponse.from_orm(job.collection.pipeline) if job.collection.pipeline else None,
        photo_extensions=await get_team_photo_extensions(job.team_id),
        camera_mappings=await get_team_camera_mappings(job.team_id),
        processing_methods=await get_team_processing_methods(job.team_id),
    )
```
...
```python
# Before: Tool assumes local config or DB connection
def run_photostats(collection_path: str, config_path: str | None = None):
    if config_path:
        config = YamlConfigLoader(config_path)
    else:
        config = DatabaseConfigLoader(get_db_session())
    # ... run analysis

# After: Tool accepts any ConfigLoader implementation
def run_photostats(
    collection_path: str,
    config_loader: ConfigLoader,  # Abstract interface
    progress_callback: Callable[[Progress], None] | None = None
) -> AnalysisResult:
    """Run PhotoStats analysis.

    Args:
        collection_path: Path to photo collection
        config_loader: Configuration source (YAML, DB, or API)
        progress_callback: Optional callback for progress updates
    """
    photo_extensions = config_loader.get_photo_extensions()
    camera_mappings = config_loader.get_camera_mappings()
    # ... run analysis using config from loader
```
...
class AgentToolExecutor:
    """Executes tools with API-based configuration."""

    def __init__(self, api_client: AgentAPIClient):
        self.config_loader = ApiConfigLoader(api_client)

    async def execute_job(self, job: Job) -> AnalysisResult:
        """Execute job using API-fetched configuration."""

        # 1. Fetch job-specific configuration (single API call)
        job_config = await self.api_client.get(f"/api/jobs/{job.guid}/config")

        # 2. Create config loader with pre-fetched data
        config_loader = PreloadedConfigLoader(job_config)

        # 3. Resolve collection path
        collection_path = await self.resolve_collection_path(
            job_config.collection,
            job.connector_credentials  # If agent has local credentials
        )

        # 4. Execute tool with config loader
        if job.tool == "photostats":
            result = run_photostats(
                collection_path=collection_path,
                config_loader=config_loader,
                progress_callback=self.report_progress
            )
        elif job.tool == "photo_pairing":
            result = run_photo_pairing(
                collection_path=collection_path,
                config_loader=config_loader,
                progress_callback=self.report_progress
            )
...
| Phase | Config Mode | Description |
|-------|-------------|-------------|
| Current | YAML, DB | Tools work with local files or direct DB |
| Phase 1 | +Interface | Abstract ConfigLoader interface, existing loaders implement it |
| Phase 2 | +API Loader | ApiConfigLoader implementation for agents |
| Phase 3 | +Endpoints | Server exposes /api/config/* and /api/jobs/{guid}/config |
| Phase 4 | Agent Use | Agents use ApiConfigLoader for all job execution |

**Backward Compatibility:** Existing CLI usage (`python photo_stats.py /path`) continues to work with YAML config. Server-side execution continues to use DatabaseConfigLoader.
...
#### FR-520: Agent Trust and Attestation

- **FR-520.1**: Agent sends binary checksum (self-hash) during registration
- **FR-520.2**: Server maintains release manifest with checksums per version/platform
- **FR-520.3**: Registration fails if binary checksum doesn't match known release
- **FR-520.4**: Agent receives job-specific signing secret when claiming a job
- **FR-520.5**: Results must include HMAC signature computed with signing secret
- **FR-520.6**: Server verifies HMAC signature before accepting results
- **FR-520.7**: Agents can be revoked (status=REVOKED), preventing job claims
- **FR-520.8**: Revoked agent's in-progress jobs released back to queue
- **FR-520.9**: Release manifest updated automatically from CI/CD pipeline
- **FR-520.10**: (v2) Support for code-signed binaries on macOS/Windows

#### FR-530: Result Ingestion Protocol

- **FR-530.1**: Results JSON < 1MB submitted inline with completion request
- **FR-530.2**: Results JSON ≥ 1MB use chunked upload protocol
- **FR-530.3**: HTML reports always use chunked upload (typically > 100KB)
- **FR-530.4**: Server specifies chunk size (default 5MB, max 10MB)
- **FR-530.5**: Chunks uploaded with PUT to `/uploads/{upload_id}/{chunk_index}`
- **FR-530.6**: Upload finalization verifies SHA-256 checksum
- **FR-530.7**: Upload sessions expire after 1 hour of inactivity
- **FR-530.8**: Chunks are idempotent (re-upload same chunk returns 409)
- **FR-530.9**: Results JSON validated against tool-specific JSON schema
- **FR-530.10**: HTML reports validated for structure and security (no external scripts)
- **FR-530.11**: Invalid results rejected with detailed error message
- **FR-530.12**: HTML report must contain generator meta tag identifying photo-admin

#### FR-540: API-Based Tool Configuration

- **FR-540.1**: ConfigLoader interface abstracts configuration source
- **FR-540.2**: YamlConfigLoader implements interface for CLI usage
- **FR-540.3**: DatabaseConfigLoader implements interface for server-side execution
- **FR-540.4**: ApiConfigLoader implements interface for agent execution (NEW)
...
- **2026-01-15 (v1.4)**: Added agent trust, result ingestion, and API-based configuration
  - **Agent Trust & Attestation**: Verify agents are from official releases
    - Binary checksum verification during registration
    - Release manifest maintained by CI/CD pipeline
    - Job-specific signing secrets for result HMAC signatures
    - Agent revocation capability with job reassignment
  - **Result Ingestion Protocol**: Handle large results safely
    - Chunked upload for HTML reports (> 100KB) and large JSON
    - SHA-256 checksum verification on reassembly
    - JSON schema validation for results
    - HTML security validation (no external scripts)
    - Retry and resumption support for reliability
  - **API-Based Tool Configuration**: Enable agent execution without DB access
    - ConfigLoader interface abstracts YAML/DB/API sources
    - New `/api/config/*` endpoints for agent consumption
    - `/api/jobs/{guid}/config` bundles all job config in single request
    - Tool refactoring to accept ConfigLoader parameter
    - Backward compatible with existing CLI and server usage
  - Added functional requirements (FR-520, FR-530, FR-540)
