...
Create `/opt/shuttersense/scripts/production-cleanup.sh`:

```bash
mkdir -p /opt/shuttersense/scripts
cat > /opt/shuttersense/scripts/production-cleanup.sh << 'CLEANUP_EOF'
#!/bin/bash
#
...
|----------|---------------|-------------|
| Dev tools | `.claude/`, `.specify/`, `.vscode/` | ~200 KB |
