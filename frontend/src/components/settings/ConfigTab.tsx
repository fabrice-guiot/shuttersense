 ...
 * Extracted from ConfigurationPage for use within SettingsPage tabs.
 */

import { useState, useEffect, useRef } from 'react'
import { Upload, Download, Camera, FileCode, Cog, Plus, Trash2, Pencil } from 'lucide-react'
...
export function ConfigTab() {
  ...
  const handleEdit = (category: ConfigCategory, key: string, value: unknown) => {
    ...
    if (category === 'cameras') {
      ...
      setEditFormData({ description: desc })
    } else if (category === 'extensions') {
      const extArray = Array.isArray(value) ? [...value] : []
      setEditFormData({ extensions: extArray })
