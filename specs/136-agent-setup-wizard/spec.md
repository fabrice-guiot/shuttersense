...
- **FR-039**: The wizard MUST display the signed download link alongside the "Download Agent" button, with a "Copy Link" action and a label indicating the link's time-to-live (e.g., "Link valid for 1 hour"). The link's TTL MUST default to 1 hour and MAY be overridden by the server via the `agent_download_signed_url_ttl` configuration setting (integer, in seconds, default `3600`). The link MUST expire after the configured TTL and become unusable afterward.
